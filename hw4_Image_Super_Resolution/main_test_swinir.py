...
def define_model(args):
    ...
    # 004 grayscale image denoising
        ...
        model = net(
            upscale=1, in_chans=1, img_size=128, window_size=8, img_range=1., depths=[
                6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[
                6, 6, 6, 6, 6, 6], mlp_ratio=2, upsampler='', resi_connection='1conv')
        param_key_g = 'params'

    # 005 color image denoising
    elif args.task == 'color_dn':
        model = net(
            upscale=1, in_chans=3, img_size=128, window_size=8, img_range=1., depths=[
                6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[
                6, 6, 6, 6, 6, 6], mlp_ratio=2, upsampler='', resi_connection='1conv')
        param_key_g = 'params'
    ...
    # because it's sligtly better than 1
        ...
        model = net(
            upscale=1, in_chans=1, img_size=126, window_size=7, img_range=255., depths=[
                6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[
                6, 6, 6, 6, 6, 6], mlp_ratio=2, upsampler='', resi_connection='1conv')
        param_key_g = 'params'
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        folder = args.folder_lq  # folder_gt
        border = args.scale
        window_size = 8
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        img_gt = None
        img_lq = cv2.imread(
            f'{args.folder_lq}/{imgname}{imgext}',
    ...
    # 004 grayscale image denoising (load gt image and generate lq image
    # on-the-fly)
    elif args.task in ['gray_dn']:
        img_gt = cv2.imread(
            path, cv2.IMREAD_GRAYSCALE).astype(
            np.float32) / 255.
    ...
    # 005 color image denoising (load gt image and generate lq image
    # on-the-fly)
    ...
    # 006 JPEG compression artifact reduction (load gt image and generate lq
    # image on-the-fly)
    elif args.task in ['jpeg_car']:
